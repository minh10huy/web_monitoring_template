...
// Connect to Database
    ...
    function checkWarning($ccCode, $today, $time) {
        ...
        try {
            ...
            $stmt = $con->prepare("SELECT appl_id FROM db_16008_c001_bak_cd2w_20170112.tbl_pega_status where cc_code = '$ccCode' and insert_date = '$today'");
            $stmt->execute();
            $cursors = $stmt->fetchAll();
            $stmt->closeCursor();
    ...
    function getWarningCDTW($ccCode, $today) {
        $proj_schema = $this->conf->digi_soft_dbschema;
        $resultDataReport = array();
        ...
        try {
            ...
            // get the last time pushing by this sale
            $stmt = $con->prepare("SELECT appl_id, client_name, id_no, status_f1, remarks FROM db_16008_c001_bak_cd2w_20170112.tbl_pega_status where cc_code = '$ccCode' and insert_date = '$today'");
            $stmt->execute();
            $cursors = $stmt->fetchAll();
            $stmt->closeCursor();
            $resultDataReport = $cursors;
    ...
    function getCDTWAppFromTo($ccCode, $fromdate, $todate) {
        $proj_schema = $this->conf->digi_soft_dbschema;
        $resultDataReport = array();
        //echo "select folder_customer_name, filename, reason_bad, notes, folder_customer_cmnd, createdtime from $proj_schema.management where cc_code_from_zip = '" . $ccCode . "' and createdtime >= '" . $today . " 00:00:00" . "' and bad_status > 0 order by createdtime DESC;";die;
        try {
            ...
            // get the last time pushing by this sale
            $stmt = $con->prepare("SELECT appl_id, client_name, id_no, status_f1, remarks FROM db_16008_c001_bak_cd2w_20170112.tbl_pega_status where cc_code = '$ccCode' and insert_date >= '$fromdate' and insert_date <= '$todate'");
            $stmt->execute();
            $cursors = $stmt->fetchAll();
            $stmt->closeCursor();
            $resultDataReport = $cursors;
