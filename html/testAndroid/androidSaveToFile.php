...
if ($upType == $newApp) {
    if (isset($_FILES['upFile'])) {
        if (move_uploaded_file($_FILES['upFile']['tmp_name'], "uploads/mobiuploads/" . $upType . '/' . $ccChannel . "/" . $ccCode . "/" . $_FILES['upFile']['name'])) {
            $res = $dataObj->insertUploadInfo($ccCode, $_FILES['upFile']['name'], $upType);
            if ($res == 1) {
                echo 1;
            } else {
                echo 0;
    ...
    if (isset($_FILES['upFile'])) {
        if (move_uploaded_file($_FILES['upFile']['tmp_name'], "uploads/mobiuploads/" . $upType . '/' . $ccChannel . "/" . $ccCode . "/" . $_FILES['upFile']['name'])) {
            $res = $dataObj->insertQDE($ccCode, basename($_FILES['upFile']['name']), $idf1, $cus_id, $cus_name, $reason);
            if ($res == 1) {
                echo 1;
            } else {
                echo 0;
        ...
        $res = $dataObj->insertQDE($ccCode, '', $idf1, $cus_id, $cus_name, $reason);
        if ($res == 1) {
            echo 1;
        } else {
            echo 0;
