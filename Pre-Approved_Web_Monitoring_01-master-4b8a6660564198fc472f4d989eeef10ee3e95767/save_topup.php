...
$check_present_user = "select present_user from db_16001_0001_bak_20160525.management where id = $management_id and (present_user != null or present_user != '') ";
...
$update_topup = "UPDATE db_16001_0001_bak_20160525.data SET 
	...
	birthday = '$birthday' WHERE management_id = $management_id";
...
if($branch_code_org !== $branch_code){
    $data_change_str.='Brand code:'.'*bf*'.$branch_code_org.'*at*'.$branch_code.'###';
}
...
if($name_tsa_org !== $name_tsa){
    $data_change_str.='Tên TSA:'.'*bf*'.$name_tsa_org.'*at*'.$name_tsa.'###';
}
if($cc_code_org !== $cc_code){
    $data_change_str.='Mã TSA:'.'*bf*'.$cc_code_org.'*at*'.$cc_code.'###';
}
if($cc_name_org !== $cc_name){
    $data_change_str.='Tên CC:'.'*bf*'.$cc_name_org.'*at*'.$cc_name.'###';
}
...
if($baohiem_vay_org !== $baohiem_vay){
    $data_change_str.='Bảo hiểm vay:'.'*bf*'.$baohiem_vay_org.'*at*'.$baohiem_vay.'###';
}
if($insurance_plus_org !== $insurance_plus){
    $data_change_str.='Insurance Plus:'.'*bf*'.$insurance_plus_org.'*at*'.$insurance_plus.'###';
}
if($insurance_name_org !== $insurance_name){
    $data_change_str.='Insuarance Name:'.'*bf*'.$insurance_name_org.'*at*'.$insurance_name.'###';
}
...
$insert_change_data = "INSERT INTO db_16001_0001_bak_20160525.preapprove_data_change_log(
...
if ($update_topup_result && $update_management_result && $insert_change_data_result) {
    ...
    //$conn->commit();
//    echo json_encode("select * from db_16001_0001_bak_20160525.sp_check_and_mark_bad_status_preapprove($management_id)");die;
    $stmt_check = $conn->prepare("select * from db_16001_0001_bak_20160525.sp_check_and_mark_bad_status_preapprove($management_id);");
    $stmt_check->execute();
    $conn->commit();
