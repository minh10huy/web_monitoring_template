...
switch ($report_type) {
    ...
    case "6" : // Hourly Downloaded Amount
        $columns = array(
            ...
            array('db' => 'QDE_Pega', 'dt' => 6),
            array('db' => 'PL Pre-Approve', 'dt' => 7),
            array('db' => 'PL Pega', 'dt' => 8));
        $function = "sp_get_data_report_tat_hourly_total_download_new_17";
        break;
    case "7" : // Hourly User Amount
        $columns = array(
            array('db' => 'per_hour', 'dt' => 0),
            array('db' => 'total_main_user', 'dt' => 1),
            array('db' => 'total_support_user', 'dt' => 2));
        $function = "sp_get_data_report_tat_hourly_total_user";
        break;
    case "8" : // Status
        $columns = array(
            array('db' => 'No.', 'dt' => 0),
        ...
        $function = "sp_get_data_report_tat_daily_new_17";
        break;
    ...
    case "10" : // Status
        ...
        $function = "sp_get_data_report_tat_daily_new_crc";
        break;
}

if ($report_type == "10") {
    $con = $pgSQL->getConDPO_DIGISOFT(true);
    echo json_encode(SSP::simple($con, null, $function, $request, $columns, $_REQUEST['date']));
} else {
    echo json_encode(SSP::simple($con, null, $function, $request, $columns, $_REQUEST['date']));
}
