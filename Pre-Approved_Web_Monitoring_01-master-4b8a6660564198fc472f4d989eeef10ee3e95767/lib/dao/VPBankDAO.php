...
class VPBankDAO {
    ...
    function getHistoryUpdateData($mana_id) {
    ...
    function getCusDataQDE($mana_id) {
        $proj_schema = $this->conf->digi_soft_dbschema;
    ...
    function check_mana_id_work($idF1) {
        $proj_schema = $this->conf->digi_soft_dbschema;
...
order by d.id desc";
    ...
    function getStageWarning() {
        ...
        try {
            ...
            // call the function
            $stmt = $con->prepare("select row_number() OVER () AS \"#\",
	app_id,
	(select capture_date_time from data where id = t.managementid),
	last_cas_status,
	reason_needaddinfo_qde
from mobile_app_notification t 
WHERE id in(
select max(id)
from mobile_app_notification t 
where managementid IN (select id from management m where m.form_id = 1 and date(createdtime) >= CURRENT_DATE -3 and m.capture_status = 5) 
group by managementid
)
AND last_cas_status = 'QDE' 
and reason_needaddinfo_qde = ''");
...
//                    "'" . $idF1 . "', '" . $content_request . "', '" . $post_actor . "', '" . $post_from . "')");
    ...
    function insertQDE($cc_code, $file_name = '', $idF1, $cus_id, $cus_name, $reason, $type, $error_code, $qde_type_code = '') {
        ...
        try {
            ...
            $stmt = null;
            // call the function
//            echo json_encode("insert into " . $proj_schema . ".upload_qde" . "(cc_code, file_name, id_f1, cus_name, cus_id_number, reason, error_code, is_pre_approved, qde_type, qde_type_code) VALUES (" .
//                        "'" . $cc_code . "', '" . $file_name . "', '" . $idF1 . "', '" . pg_escape_string($cus_name) . "', '" . $cus_id . "', '" . $reason . "','" . $error_code . "', 1, 1, '" . $qde_type_code . "')");die;
            if ($type == 3) {
