...
class LookupChannelDataDAO {
    ...
    public function getChannels($rows, $offset, $filter, $sort, $order, $id = NULL){
        ...
        $where = " WHERE lower(lc.company) like 'vpbank' AND lc.cc_role NOT IN ('admin', 'subadmin', 'manager') AND lc.is_enable = true";
        if (!is_null($id)) {
...
//        echo $query;exit();
    ...
    public function deleteChannels($id){
        ...
        $select = "update db_16001_0001_bak_20160525.lookup_channel set is_enable = false, cc_password = md5('user da xoa') where id = $id";
...
//        echo $query;exit();
    ...
    public function insertIMEI($device_model, $imei_no, $notes){
        $conn = PostgreSQLClass::getConn();
        // declare my query and execute
        $select = "UPDATE db_16001_0001_bak_20160525.lookup_channel SET security_key = concat(security_key, '/".$imei_no."') WHERE cc_code = '".$device_model."'";
