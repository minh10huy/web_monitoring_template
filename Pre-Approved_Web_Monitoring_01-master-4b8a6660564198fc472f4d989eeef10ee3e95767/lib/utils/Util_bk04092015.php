...
//                        $table.find('tr:gt(0)').append('<td style="white-space: nowrap; width: 1%;">' + toolbar + '</td>');
        for ($i = 0; $i < count($dataReportGeneral); $i++) {
            ...
            foreach ($dataReportGeneral[$i] as $key => $value) {
                if ($tableId === 2 && $type===3) {
                    if ($key === "DSA Code" ||$key === "DSA Name" || $key === "TSA Code F1" || $key === "Spouse Name" || $key === "TSA Name" || $key === "TSA Phone Number" || $key === "Product Name 1" || $key === "Loan Amount Request" || $key === "Loan Term Request" || $key === "Insurance" || $key === "Date of Closure" || $key === "Disb Channel" || $key === "Branch Code" || $key === "Referee 1" || $key === "Referee 2" || $key === "Spouse Name" || $key === "CC Code" || $key === "CC Name" || $key === "Opportunity Name" || $key === "No. Agreement ID" || $key === "New ID Card Number" || $key === "Date of Issue" || $key === "Place of Issue" || $key === "New Phone" || $key === "Address" || $key === "Actual Address" || $key === "Monthly Income" || $key === "Monthly Costs" || $key === "Monthly Income Family"|| $key === "Monthly Costs Family"|| $key === "Number of Modified Fields" || $key === "Modified Fields")
                        $resultDataReportGeneral .= '<td class="tabledit-view-mode"><span class="tabledit-span">' . $value .'</span><input type="text" disabled="" style="display: none;" value="' . $value . '" name="'. $key .'" class="tabledit-input form-control input-sm"></td>';
