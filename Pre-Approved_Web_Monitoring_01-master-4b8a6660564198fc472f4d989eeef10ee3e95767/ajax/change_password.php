...
if (isset($_POST['username'])) {
    ...
    if(!$user->checkLogin($username, $oldPass)){
        echo "error";
        exit();
    }
    if(!preg_match("/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[^a-zA-Z0-9])(?!.*\s).{8,15}$/", $newPass)){
    ...
    // Đổi password mới
    if($vpBank->changePassword($username, $newPass, $email)){
        $_SESSION['insecure_pass'] = "false";
        echo "success";
        exit();
    }
    echo "error";
