...
$update_insurance_plus = isset($_POST['Insurance_Plus']) ? $_POST['Insurance_Plus'] : "";//Mui add
$update_insurance_name = isset($_POST['Insurance_Name']) ? $_POST['Insurance_Name'] : "";//Mui add
...
$update_topup = "UPDATE db_16001_0001_bak_20160525.pre_approve_topup SET sodienthoai_tsa =:update_tsa_phone_number, product_name_1 =:update_product_name, product_code_1 =:update_product_code_1, sotienvay =:update_loan_amount_request, 
                                    ...
                                    baohiem_vay =:update_insurance, insurance_plus =:update_insurance_plus, insurance_name =:update_insurance_name, tenkhachhang =:update_opportunity_name, so_id_cu =:update_no_agreement_id, so_cmnd =:update_id_card_no, description =:update_description,
                                    ...
                                    thunhap_kh_bsung =:update_monthly_income, chiphi_kh_bsung =:update_monthly_costs, sdt_kh_bsung =:update_phone_no, no_modified_fields =:update_no_modified_fields, monthly_income_family =:update_monthly_income_family, 
                                        monthly_costs_family =:update_monthly_costs_family, modified_fields =:update_modified_fields WHERE management_id = $update_management_id";
//, insurance_plus =:update_insurance_plus, insurance_name =:update_insurance_name    
$prepare = $conn->prepare($update_topup);
$update_topup_result = $prepare->execute(array(':update_tsa_phone_number' => $update_tsa_phone_number, ':update_product_name'=> $update_product_name, ':update_product_code'=> $update_product_code, ':update_loan_amount_request'=> $update_loan_amount_request, 
                                    ...
                                    ':update_insurance'=> $update_insurance, ':update_insurance_plus'=> $update_insurance_plus, ':update_insurance_name'=> $update_insurance_name, ':update_opportunity_name'=> $update_opportunity_name, ':update_no_agreement_id'=> $update_no_agreement_id, ':update_id_card_no'=> $update_id_card_no, ':update_description'=> $update_description,
                                    ...
                                    ':update_monthly_income'=> $update_monthly_income, ':update_monthly_costs'=> $update_monthly_costs, ':update_phone_no'=> $update_phone_no, ':update_no_modified_fields'=> $update_no_modified_fields, ':update_monthly_income_family'=> $update_monthly_income_family, 
                                        ':update_monthly_costs_family'=> $update_monthly_costs_family, ':update_modified_fields' => $update_modified_fields));
...
$update_management = "UPDATE db_16001_0001_bak_20160525.management SET update_infor_times = update_infor_times + 1 WHERE id = $update_management_id";
...
$update_download_time = "UPDATE db_16001_0001_bak_20160525.pre_approve_topup SET updated_time = '$current_time' WHERE management_id = $update_management_id";
$prepare = $conn->prepare($update_download_time);
$update_download_result = $prepare->execute();

// Begin update table Data
$update_data = "SELECT * FROM db_16001_0001_bak_20160525.sp_savejob_bad_image_verify($update_management_id, 4, 0, NULL, 'update from not ok', NULL);";
...
$check_valid = 'SELECT * FROM db_16001_0001_bak_20160525.sp_check_and_mark_bad_status_preapprove(' . $update_management_id. ')';
$prepare = $conn->prepare($check_valid);
$valid_result = $prepare->execute();
$result_data = $prepare->fetchAll();

if (strlen($result_data[0]['sp_check_and_mark_bad_status_preapprove']) > 0) {
    echo json_encode(array('result' => 'Invalid', 'reason' => $result_data[0]['sp_check_and_mark_bad_status_preapprove']));
    die();
}
